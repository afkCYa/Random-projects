...
//		System.out.println("");		
		...
		String exit = "EXIT";
			...
			System.out.println("If you write it wrong you will get kicked from the program");
		...
		if(action.equalsIgnoreCase(exit)) {
			...
			try (FileReader fr = new FileReader(filename);
				...
				while((line = br.readLine()) != null) {
					if (linenum == 1) {
						...
						pl = new Player(nameblack, namewhite);
						if (line.equals("@")) {
							pl.setPlayerB();
						}
						else {
			...
			keyb.nextLine();//eat the rest of the line
			bd = new BoardChange(boardchoice);
		}//ELSE IF for new game
		
		else {
			System.out.println("Invalid output, bye");
			System.exit(0);
		}
				...
				System.out.println("You can ask for a HINT for your moves");
				System.out.println("You may also SKIP turns, SURRENDER, or EXIT the game");
			bd.printBoard();
			char car;
			String cars;
			String carp;
			...
			while (skip != 2) {
					...
					System.out.println("Separate them with one space then hit enter");
//					System.out.println("Please again avoid missinputting and crashing the program");
//					fixed it hehe
				cars = keyb.nextLine();
				Square sq = null;
				if (cars.equalsIgnoreCase("SKIP")) {
					System.out.println("Turn skipped");
					pl.skipTurn();
					skip = skip + 1;
				} else if(cars.equalsIgnoreCase(exit)) {
					System.out.println("Saving and leaving");
					leaving = 1;
					break;
				} else if (cars.equalsIgnoreCase("SURRENDER")) {
					surrender = 1;
					break;
				} else if (cars.equalsIgnoreCase("HINT")) {
					Validation.checkSkippables(bd, sq, pl);
				} else {
					try {
						car = cars.charAt(0);
						carp = cars.substring(2);
						mint = Integer.parseInt(carp);
						sq = new Square(car, mint);
						...
						System.out.println("Please enter your move in the format of: letter NUMBER");
						System.out.println("Separate them with ONE  s p a c e  then hit enter");
					}
				}//skip else
			}//while skip
			bd.printBoard();
			...
			try {
				...
				keyb.close();
				System.out.println("Something broke, maybe " + e.getStackTrace());
			}
